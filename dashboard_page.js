...
function updateProtectionStatus(packetInfoList) {
    ...
    if (statusText && dotImg && threatStatus) {
        if (safeCount > suspiciousCount) {
            ...
            statusText.textContent = 'Unprotected';// Get IP address from localStorage
    const ipAddress = localStorage.getItem('dashboardIPAddress');
    
    if (!ipAddress) {
        console.error('No IP address found. Please go back to download page and enter IP address.');
        showMessageStatus('No IP address found. Please go back to download page.', false);
        return;
    }
...
function fetchDashboardData() {
    // Get IP address from localStorage
    const ipAddress = localStorage.getItem('dashboardIPAddress');
    ...
    Promise.all([
        fetch('https://apiget.huzaifa.cloud/api/dashboard-packet-data', {
            method: 'POST',
            headers: {
         ...
         fetch('https://apiget.huzaifa.cloud/api/dashboard-loadconn-data', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
    ...
    .catch(err => {
        ...
        showMessageStatus('Error fetching dashboard data. Please check your connection.', false);
    });
}

// Function to show IP address missing modal
function showIPAddressMissingModal() {
    let ipMissingModal = document.getElementById('ip-missing-modal');
    if (!ipMissingModal) {
        ipMissingModal = document.createElement('div');
        ipMissingModal.id = 'ip-missing-modal';
        ipMissingModal.style.position = 'fixed';
        ipMissingModal.style.top = '0';
        ipMissingModal.style.left = '0';
        ipMissingModal.style.width = '100vw';
        ipMissingModal.style.height = '100vh';
        ipMissingModal.style.background = 'rgba(0,0,0,0.7)';
        ipMissingModal.style.zIndex = '10050';
        ipMissingModal.style.display = 'flex';
        ipMissingModal.style.justifyContent = 'center';
        ipMissingModal.style.alignItems = 'center';
        ipMissingModal.innerHTML = `
            <div style="background:#fff; padding:32px 24px; border-radius:12px; box-shadow:0 4px 24px rgba(0,0,0,0.18); max-width:90vw; width:400px; text-align:center; position:relative;">
                <h3 style="font-size:1.25rem; font-family:Poppins,Inter,sans-serif; color:#222; margin-bottom:16px;">IP Address Required</h3>
                <p style="font-size:1rem; font-family:Poppins,Inter,sans-serif; color:#666; margin-bottom:20px; line-height:1.5;">
                    No IP address found. Please go back to the download page and enter your IP address.
                </p>
                <div style="display:flex; gap:12px; justify-content:center;">
                    <button id="go-back-btn" style="padding:10px 20px; background:#007bff; color:#fff; border:none; border-radius:6px; font-size:1rem; cursor:pointer;">Go to Download Page</button>
                </div>
            </div>
        `;
        document.body.appendChild(ipMissingModal);
...
document.addEventListener('DOMContentLoaded', function() {
     // Check if IP address exists in localStorage
    const ipAddress = localStorage.getItem('dashboardIPAddress');
    if (!ipAddress) {
        showIPAddressMissingModal();
        return; // Don't proceed with dashboard initialization
    }
