...
// Function to show progress bar after API activation
function showProgressBarAfterActivation() {
  // First check for uninstalled packages
  fetch('https://apiget.huzaifa.cloud/api/check-uninstalled-packages')
    .then(resp => resp.json())
    .then(data => {
      if (data.status === 'need_installation') {
    ...
    .catch(() => {
      // Default to progress bar on error
      showProgressBarInternal();
    });
}

// Internal function to show progress bar
function showProgressBarInternal() {
